...
class ObsRecorderNode(Node):
    ...
    def _acquire_process_lock(self):
        lock_path = '/tmp/ignacio_obs_recorder.lock'
        ...
        try:
            ...
            lock_file.close()
            self.get_logger().error(
                'Another obs_recorder_node instance is already running. Exiting.')
            raise RuntimeError('obs_recorder_node already running')
    ...
    def _start_recording(self) -> tuple[bool, str]:
        ...
        self._sync_state()
        if self._recording:
            return True, f'Already recording → {self._save_dir}'
        ...
        try:
            ...
            err = str(e)
            # OBS can return code 500 when recording is already active.
            # Treat it as success if status confirms active recording.
            self._sync_state()
            if self._recording or 'code 500' in err or 'already' in err.lower():
                self._recording = True
                self.get_logger().warn(
                    f'OBS start returned warning ({err}) but recording is active.')
                return True, f'Already recording → {self._save_dir}'
