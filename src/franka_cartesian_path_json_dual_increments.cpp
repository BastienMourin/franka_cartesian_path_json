    ...
    const std::vector<std::string>    &extra_ros_args = {})
{
    ...
    if (pid == 0)
    {
        // Put each launched recorder in its own process group so parent can
        // reliably stop the whole subtree with kill(-pid, ...).
        setpgid(0, 0);

        // Ensure child receives SIGINT if parent dies unexpectedly
...
void stopChildProcess(pid_t pid, const rclcpp::Logger &logger, const std::string &name = "", int timeout_ms = 3000)
{
    ...
    RCLCPP_INFO(logger,
        "Sending SIGINT to %s (PID %d / PGID %d) — waiting for cleanup...", name_str.c_str(), pid, pid);

    // Signal the whole process group so subprocess trees are stopped too.
    // This handles cases where 'ros2 run' exits but spawned Python node survives.
    if (kill(-pid, SIGINT) != 0) {
        // Fallback to single-PID signal if process group no longer exists.
        kill(pid, SIGINT);
    }
    ...
    RCLCPP_WARN(logger,
        "%s (PID %d / PGID %d) did not exit within %d ms — sending SIGKILL.",
        name_str.c_str(), pid, pid, timeout_ms);
...
// ---------------------------------------------------------------------------
// Kill stale recorder processes from previous runs.
// Uses pgrep -f to find matching command lines, excluding this process.
// ---------------------------------------------------------------------------
std::vector<pid_t> findPidsByPattern(const std::string &pattern, pid_t self_pid)
{
    ...
    while (fgets(buffer.data(), static_cast<int>(buffer.size()), pipe) != nullptr) {
        try {
            pid_t pid = static_cast<pid_t>(std::stol(buffer.data()));
            if (pid > 0 && pid != self_pid) pids.push_back(pid);
        } catch (...) {
...
void cleanupStaleRecorderProcesses(const rclcpp::Logger &logger)
{
    const pid_t self_pid = getpid();

    const std::vector<std::pair<std::string, std::string>> targets = {
        {"recording_manager_node", "recording_manager"},
        {"realsense_recorder_node", "realsense_recorder"},
        {"obs_recorder_node", "obs_recorder"},
        {"wrench_logger_node", "wrench_logger"}
    };

    for (const auto &[pattern, name] : targets) {
        auto pids = findPidsByPattern(pattern, self_pid);
        for (pid_t pid : pids) {
            if (pid <= 0) continue;

            RCLCPP_WARN(logger,
                "Found stale %s process (PID %d). Sending SIGINT...", name.c_str(), pid);
            kill(pid, SIGINT);

            bool exited = false;
            for (int i = 0; i < 20; ++i) {  // wait up to ~2 s
            ...
            if (!exited) {
                RCLCPP_WARN(logger,
                    "%s PID %d still alive after SIGINT. Sending SIGKILL...", name.c_str(), pid);
                kill(pid, SIGKILL);
            } else {
                RCLCPP_INFO(logger,
                    "Stale %s process (PID %d) exited.", name.c_str(), pid);
            }
        }
    }
}
    ...
    DebugPublishers debug_pubs)
{
    ...
    for (const auto &wp : handle["waypoints"])
    {
        ...
        {
            RCLCPP_WARN(node->get_logger(),
                "[handle %d] Skipping waypoint at t=%.2f: could not rotate delta position.",
                handle_id, t);
            continue;
        }
        ...
        {
            RCLCPP_WARN(node->get_logger(),
                "[handle %d] Skipping waypoint at t=%.2f: could not rotate delta orientation.",
                handle_id, t);
            continue;
        }
...
int main(int argc, char **argv)
{
    ...
    if (enable_recording && !debug) {
        ...
        // Clean up stale recorder processes from previous interrupted runs
        cleanupStaleRecorderProcesses(node->get_logger());
