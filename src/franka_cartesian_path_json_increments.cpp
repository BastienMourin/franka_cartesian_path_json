...
geometry_msgs::msg::Pose applyDeltaPose(
    ...
    const rclcpp::Logger           &logger)
{
    // Position: direct addition — delta is already in fr3_link0
    Eigen::Vector3d new_position(
        base.position.x + delta.position.x,
        base.position.y + delta.position.y,
        base.position.z + delta.position.z);

    // Orientation: pre-multiply to apply delta in world frame (fr3_link0)
...
int main(int argc, char **argv)
{
    ...
    const std::string CONTROL_FRAME = "fr3_link0";
    const std::string TCP_FRAME     = "fr3_hand_tcp";
    ...
    node->declare_parameter("waypoints_file", "waypoints_test_increments.json");
    std::string filename =
    ...
    json j;
    file >> j;
    RCLCPP_INFO(node->get_logger(), "Loaded JSON file: %s", filename.c_str());

    // ----------------------------------------------------------
    // 4. Capture the starting TCP pose in fr3_link0 ONCE.
    //    Every waypoint delta will be applied on top of this pose,
    //    so the trajectory is always relative to wherever the robot
    //    starts — regardless of its initial configuration.
    // ----------------------------------------------------------
    geometry_msgs::msg::PoseStamped start_pose_in_link0;
    if (!getCurrentPose(TCP_FRAME, CONTROL_FRAME,
                        start_pose_in_link0, *tf_buffer, node->get_logger()))
    {
    ...
    // ----------------------------------------------------------
    // 5. Process handles and waypoints
    // ----------------------------------------------------------
    for (const auto &handle : j["handles"])
    {
        ...
        std::string frame_id  = handle["frame_id"].get<std::string>();

        RCLCPP_INFO(node->get_logger(),
            "=== Processing handle %d (frame: %s) ===", handle_id, frame_id.c_str());
        ...
        for (const auto &wp : handle["waypoints"])
        {
            ...
            // --- Step A: Rotate delta POSITION into fr3_link0 ---
            // Reexpresses the displacement direction using rotation only.
            // No origin offset is added (Vector3Stamped semantics).
            geometry_msgs::msg::Point delta_pos_in_link0;
            if (!rotateDeltaPosition(raw_delta_pos, frame_id, CONTROL_FRAME,
                                     delta_pos_in_link0, *tf_buffer,
            ...
            // --- Step B: Re-express delta ORIENTATION into fr3_link0 ---
            // Uses the similarity transform q_rot * q_delta * q_rot^{-1}
            // to rotate the axis of the delta into fr3_link0 coordinates
            // without adding any orientation offset from the source frame's
            // own pose. Identity input always produces identity output.
            ...
            // --- Step C: Build delta pose in fr3_link0 ---
            ...
            // --- Step D: Apply delta on top of the starting TCP pose ---
            // Position:    start.position + delta.position  (world-frame addition)
            // Orientation: q_delta * start.orientation      (world-frame pre-multiply)
    ...
    RCLCPP_INFO(node->get_logger(), "All handles processed. Shutting down.");
    rclcpp::shutdown();
    return 0;
}